...
int beta, Nmax, Lanmax, Blocks, Spinmax, nset, nctot, Powns, Powns2, myrank, size, nb, tnb, Njj, Ndd, tol_small, Nonzero_SOC, ROTATE, beta_real = 0, Vmax = SDmax, AF=0;
...
int main(int argc, char *argv[]){
	...
	if(argc != 17 && argc != 19){
		printf("Usage :: %s <inputfilename> <Hz> <JTD> <SOC> <J> <U_init> <U_final> <DELTA> <Lanmax> <beta> <Nmax> <ratio new/old> <Repeat> <Interval> <tol_small> <AFcode> <optional: forced_gindex> <optional: forced_ndeg>\n", argv[0]);
		printf("Usage :: %s <inputfilename> <Hz> <JTD> <SOC> <J> <U_init> <U_final> <DELTA> <Lanmax> <beta> <Nmax> <ratio new/old> <Repeat> <Interval> <tol_small> <AFcode>  <optional: beta_real> <optional: number_max_states>\n", argv[0]);
		exit(1);
	}
	...
	if( argc == 19 ){
		...
		if( i > 10 ){
			...
			forced_gindex = atoi( argv[17] );
			forced_ndeg = atoi( argv[19] );
		}
	}
	...
	if( forced_gindex ){
		sprintf(tails,
				"%cDir_OPT%dVec%d_UF%.2lf_UT%.2lf_ns%d_nc%dnb%d_beta%d_Nmax%d_AF%d_Hz%.3lf_JT%.3lf_S%.3lf_J%.3lf_D%.3lf_mix%.2lf_tol%d_Nint%d_n%d_R%d_ts%d_fg%d_fd%d",
				Shead, Tolgreen, Nvec, U_init, U_final, Ns, nctot, nb, beta, Nmax, AF, Hz, JTD, SOC, J, DELTA, ratio, tol, Nintx, size, ROTATE, tol_small, forced_gindex, forced_ndeg
		       );
	}
	else{
		sprintf(tails,
				"%cDir_OPT%dVec%d_UF%.2lf_UT%.2lf_ns%d_nc%dnb%d_beta%d_Nmax%d_AF%d_Hz%.3lf_JT%.3lf_S%.3lf_J%.3lf_D%.3lf_mix%.2lf_tol%d_Nint%d_n%d_R%d_ts%d_IT%d_Ne%d",
				Shead, Tolgreen, Nvec, U_init, U_final, Ns, nctot, nb, beta, Nmax, AF, Hz, JTD, SOC, J, DELTA, ratio, tol, Nintx, size, ROTATE, tol_small, beta_real, Vmax
		       );
	}
	char fname[1024] = "inputs/LinG.txt";
	fp = fopen(fname, "r");
	if( fp != NULL )	sprintf(tails, "%s_GL", tails);
	...
	double new_MU = Efermi + 3.5*Uinter - 8*J + DELTA;
	if( fabs(Uinter - U_init) < 1e-6 ) for(n=0; n<Ni; n++){
		//shift_bath(&egv[n], new_MU-MU);
		//if( myrank == 0 ) print_egv("shifted", egv[n]);
	}
	...
	for(i=0; i<NU; i++)	transform[i][i] = gsl_complex_rect(1,0);

	gsl_complex **rot;
	if(fp != NULL){
		sprintf(para, "cp %s %s/", fname, save_directory);
		system(para);
	...
	for(iu=0; iu<iumax; iu++){
		...
		//MU = Efermi + 3.5*Uinter - 8*J + DELTA;
		MU = Efermi + set_chem_pot(3, Uinter, J) + DELTA;
		init_Ecluster( Ecluster, hopmatrix, &quadrature );
		update_chem(Ecluster);
		...
		do{
			...
			if( myrank == 0 ){
				if( count == count_init ) for(n=0; n<Ni; n++) for(i=0; i<Nmax; i++) for(j=0; j<tNC; j++) for(k=0; k<tNC; k++)	Greenold[n][i][j][k] = Greennew[n][i][j][k];
			...
			if( titer == 1 || count == Repeat){
				...
				if( myrank == 0 ){
					...
					fprintf(fresult, "./$1 \t%s/%s \t%.3lf\t \t%.3lf \t %.6lf \t%.3lf \t%.3lf \t%.3lf \t%.3lf \t%s \t%d \t%d\n", tails, saveEV, Hz, JTD, SOC, J, U_init, U_final, DELTA, tails, beta_real, AF);
					fclose( fresult );
					fclose(fmain);
					fclose(fp);
					if( Post_process ){
						sprintf(para, "./static %s/%s \t%.3lf\t %.3lf %.6lf %.3lf %.3lf %.3lf %.3lf %s %d %d", tails, saveEV, Hz, JTD, SOC, J, U_init, U_final, DELTA, tails, beta_real, AF);
						system(para);
						printf("static: Done\n");	fflush(stdout);
						sprintf(para, "./vert_new %s/%s \t%.3lf\t %.3lf %.6lf %.3lf %.3lf %.3lf %.3lf %s %d %d", tails, saveEV, Hz, JTD, SOC, J, U_init, U_final, DELTA, tails, beta_real, AF);
						system(para);
						printf("vert_new: Done\n");	fflush(stdout);
						sprintf(para, "./genG %s/%s \t%.3lf\t %.3lf %.6lf %.3lf %.3lf %.3lf %.3lf %s %d %d", tails, saveEV, Hz, JTD, SOC, J, U_init, U_final, DELTA, tails, beta_real, AF);
						system(para);
						printf("genG: Done\n");	fflush(stdout);
						sprintf(para, "./local %s/%s \t%.3lf\t %.3lf %.6lf %.3lf %.3lf %.3lf %.3lf %s %d %d", tails, saveEV, Hz, JTD, SOC, J, U_init, U_final, DELTA, tails, beta_real, AF);
						system(para);
						printf("local: Done\n");	fflush(stdout);
